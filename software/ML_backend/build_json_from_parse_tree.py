...
python build_json_from_parse_tree.py --folder test_data --result_file_name source_pascal_target_java_atom_test
"""
...
def prepare_data(pascal_str):
    ...
    java_file_name = 'java_dummy_ast_str.txt'
    ...
    while True:
        ...
        # pascal_str = "(program (programHeading program (identifier TestPascalFile) ;) (block (usesUnitsPart uses (identifierList (identifier crt)) ;) (compoundStatement begin (statements (statement (unlabelledStatement (simpleStatement (procedureStatement (identifier writeln) ( (parameterList (actualParameter (expression (simpleExpression (term (signedFactor (factor (unsignedConstant (string 'Hello, World!'))))))))) ))))) ; (statement (unlabelledStatement (simpleStatement (procedureStatement (identifier readkey))))) ; (statement (unlabelledStatement (simpleStatement emptyStatement_)))) end)) . <EOF>)"
        java_str = java_file.read()
        
        pascal_file_name = get_token_list(pascal_str)
        print(f"Num = {count}, Start to process pascal")
        pascal_dict = build_ast(pascal_file_name)
  
        java_token_list = get_token_list(java_str)
